...
const VideoTile = memo(
  ({
    ...
    return (
      <div
        className={`relative rounded-2xl overflow-hidden group shadow-xl border ${isPinned && !mini ? "border-blue-500/50 ring-2 ring-blue-500/20" : "border-gray-700/40"} bg-[#1a1c1e] h-full w-full transition-all duration-300 flex items-center justify-center`}
      >
        <video
          ref={videoRef}
          autoPlay
        ...
        {!isCamOn && !isPresentation && (
          <div className="absolute inset-0 flex items-center justify-center bg-[#282a2d] pointer-events-none border-none outline-none">
            <div
              className={`${mini ? "h-10 w-10 text-xs" : "h-16 w-16 md:h-28 md:w-28 text-xl md:text-4xl"} rounded-full bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center font-bold shadow-2xl border-2 border-white/10 uppercase`}
            >
              {name.charAt(0)}
            </div>
          </div>
        ...
        <div
          className={`absolute bottom-2 left-2 flex items-center gap-1.5 bg-black/40 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/10 ${mini ? "scale-90 origin-left" : ""} z-10 pointer-events-none`}
        >
          {!isPresentation &&
            (isMicOn ? (
        ...
        {onPin && (
          <button
            ...
            className="absolute top-2 right-2 p-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/5 hover:bg-blue-600 transition-all opacity-0 group-hover:opacity-100 z-20 pointer-events-auto"
          >
            {isPinned ? (
...
const App: React.FC = () => {
  ...
  const initPeer = (
    ...
    peer.on("open", (openedId: string) => {
      console.log(`🌐 [PeerJS] My Peer ID is: ${openedId}`);
  ...
  const joinMeeting = async () => {
    ...
    try {
      ...
      setMyId(randomId);
      initPeer(randomId, iceServers, joinCodeInput, joinNameInput);
    } catch (e) {
      setIsRequesting(false);
    }
  };
      ...
      <div className="min-h-[100dvh] flex items-center justify-center p-4 bg-[#202124] text-white">
        <div className="text-center space-y-6 max-w-sm animate-in fade-in zoom-in duration-300">
          ...
          <p className="text-gray-400">
            Harap tunggu, kami sedang mengoptimalkan rute jaringan terbaik.
          </p>
          <button
            onClick={resetApp}
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-[#202124] text-white overflow-hidden relative">
      {pendingRequests.length > 0 && (
        <div className="absolute top-16 md:top-20 right-4 z-[100] w-full max-w-[280px] space-y-3 pointer-events-none">
          {pendingRequests.map((request) => (
            <div
              key={request.id}
              className="bg-[#1a1c1e]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl animate-in slide-in-from-right duration-300 pointer-events-auto"
            >
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden p-2 md:p-3 gap-2 md:gap-3 min-h-0 relative z-10">
        <div
          className={`transition-all duration-500 ease-in-out ${isAnyPinned ? "h-[40vh] md:h-full md:flex-[4]" : "flex-1"} min-h-0 flex-shrink-0`}
        >
          {isAnyPinned ? (
            <VideoTile
              stream={
      ...
      <footer className="h-20 md:h-28 flex items-center justify-between px-4 md:px-12 bg-[#202124] flex-shrink-0 z-40 pb-4 md:pb-0 border-t border-gray-800/50">
        ...
        <div className="flex items-center justify-center flex-1 md:flex-none gap-2.5 md:gap-5">
          <button
            onClick={toggleMic}
            className={`p-3 md:p-5 rounded-full transition-all border border-gray-700/50 shadow-lg ${isMicOn ? "bg-[#3c4043] hover:bg-[#434649]" : "bg-red-500 hover:bg-red-600 ring-4 ring-red-500/20"}`}
          >
            {isMicOn ? (
          ...
          <button
            onClick={toggleCam}
            className={`p-3 md:p-5 rounded-full transition-all border border-gray-700/50 shadow-lg ${isCamOn ? "bg-[#3c4043] hover:bg-[#434649]" : "bg-red-500 hover:bg-red-600 ring-4 ring-red-500/20"}`}
          >
            {isCamOn ? (
          ...
          <button
            ...
            className={`p-3 md:p-5 rounded-full transition-all border border-gray-700/50 shadow-lg ${isScreenSharing ? "bg-blue-600 ring-4 ring-blue-600/20" : "bg-[#3c4043] hover:bg-[#434649]"}`}
          >
            <MonitorUp
              className={`h-5 w-5 md:h-6 md:w-6 ${isScreenSharing ? "text-white" : "text-gray-300"}`}
            />
          </button>
